...
The AI Chat system is a personal chat interface backed by a local [Ollama](https://ollama.com) instance. Conversations are stored in the database (sessions and messages), streamed in real time over Server-Sent Events, and rendered client-side with `marked.js` and `highlight.js`. The system supports images (base64), documents (PDF and plain text), and models that emit a separate thinking stream.
...
- **REST API** — session and message management plus the Ollama proxy; protected by `ApiFilter`
...
| `model` | VARCHAR | Ollama model name active for this session (e.g. `llama3.2`) |
...
| `thinking` | TEXT | Separated `<think>` content if the model emits it; NULL otherwise |
...
|---|---|---|
| `id` | INT PK | Auto-increment |
| `content` | TEXT | Prompt text injected as `role: system` on every Ollama request |
...
| Method | Path | Handler | Description |
|---|---|---|---|
| GET | `/api/ai/chat/models` | `Ai\Api\Chat::models` | List Ollama models from `/api/tags` |
| GET | `/api/ai/chat/sessions` | `Ai\Api\Chat::sessions` | All sessions ordered by pinned then updated |
| POST | `/api/ai/chat/session` | `Ai\Api\Chat::createSession` | Create a new session with a chosen model |
| PATCH | `/api/ai/chat/session/{uuid}` | `Ai\Api\Chat::updateSession` | Update `title`, `pinned`, or `model` |
...
The stream endpoint proxies a request to `Ollama /api/chat` and re-emits the chunks as SSE. All headers (Content-Type, Cache-Control, X-Accel-Buffering) are set to prevent buffering. `set_time_limit(300)` is applied for long responses.
...
**Session creation on first message:** If `session_uuid` is absent or the UUID is not found, a new session is inserted automatically. The title is taken from the first 60 characters of the message, or from the document/image name if the message is empty.

**Document injection:** Attached document content is prepended to the user message before being sent to Ollama, wrapped in `--- filename ---` / `--- end of filename ---` delimiters.

**Image injection:** Image data-URLs are stripped to bare base64 and passed as the `images` array in the Ollama message payload.
...
| `data` | `{thinking: "chunk"}` | Thinking-model tokens (when `message.thinking` is set) |
| `done` | `{done: true, model, created_at}` | After all tokens; `assistant` message is persisted here |
| `error` | `{error: "..."}` | Ollama connection failure or empty payload |

**Thinking extraction fallback:** After the stream completes, if `fullThinking` is empty but the assembled content begins with `<think>…</think>`, the tag is parsed out and stored in the `thinking` column. This handles models that embed thinking in `content` rather than the dedicated `thinking` field.
...
The model selector persists the chosen model to `localStorage` (`chat_model`).
...
| File type | Handling |
|---|---|
| `image/*` | Read as data-URL via `FileReader`; stored in `pendingImages`; sent as base64 to Ollama |
...
If the Ollama connection fails during a stream, the assistant bubble shows an error and a **Retry** button. On retry, the last user message, images, and documents are restored and `sendMessage()` is called again.
...
Every save creates a new row in `system_prompts` rather than overwriting. The row with the highest `id` is considered active (`SystemPromptModel::getActive()`).
...
| [app/Controllers/Ai/Api/Chat.php](app/Controllers/Ai/Api/Chat.php) | REST API: sessions, messages, stream, search, PDF extraction |
