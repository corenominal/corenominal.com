...
class Dashboard extends BaseController
{
    ...
    public function index(): string
    {
        ...
        $totalHits   = $model->countAllResults();
        $today       = $model->where('DATE(created_at)', date('Y-m-d'))->countAllResults();
        $weekStart   = date('Y-m-d', strtotime('monday this week'));
        $thisWeek    = $model->where('created_at >=', $weekStart)->countAllResults();
        $monthStart  = date('Y-m-01');
        $thisMonth   = $model->where('created_at >=', $monthStart)->countAllResults();

        $uniquePaths = (int) $db->query(
            'SELECT COUNT(DISTINCT path) AS cnt FROM metrics'
        )->getRow()->cnt;

        $uniqueIPs = (int) $db->query(
            'SELECT COUNT(DISTINCT anonymized_ip) AS cnt FROM metrics'
        )->getRow()->cnt;
        ...
        $deviceRows = $db->query(
            'SELECT device_type, COUNT(*) AS cnt FROM metrics GROUP BY device_type ORDER BY cnt DESC'
        )->getResultArray();
        $totalForPct = array_sum(array_column($deviceRows, 'cnt')) ?: 1;

        $topPaths = $db->query(
            'SELECT path, COUNT(*) AS cnt FROM metrics GROUP BY path ORDER BY cnt DESC LIMIT 10'
        )->getResultArray();
