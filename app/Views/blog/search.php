...
<?= $this->section('content') ?>
    ...
    <form class="mb-4" action="<?= site_url('blog/search') ?>" method="get" role="search">
    ...
    <?php if ($query !== ''): ?>
        ...
        <h2 class="h6 text-uppercase text-muted fw-semibold mb-3">
            <?= count($posts) === 1 ? '1 result' : count($posts) . ' results' ?> for &ldquo;<?= esc((string) $query) ?>&rdquo;
        </h2>
        ...
        <?php else: ?>
        <p class="text-muted mb-5">No posts found for &ldquo;<?= esc((string) $query) ?>&rdquo;.</p>
        <?php endif; ?>
