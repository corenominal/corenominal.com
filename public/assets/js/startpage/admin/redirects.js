document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Filter ─────────────────────────────────────────────────────────────────
  const filterInput = document.getElementById('redirects-filter');
  if (filterInput) {
    const tbody = document.querySelector('#table-redirects tbody');
    const colSpan = document.querySelector('#table-redirects thead tr')?.children.length || 4;

    const emptyRow = document.createElement('tr');
    emptyRow.id = 'redirects-filter-empty';
    emptyRow.classList.add('d-none');
    emptyRow.innerHTML = `<td colspan="${colSpan}" class="text-center text-secondary">No matching redirects.</td>`;
    tbody.appendChild(emptyRow);

    filterInput.addEventListener('input', () => {
      const query = filterInput.value.trim().toLowerCase();
      const rows = tbody.querySelectorAll('tr:not(#redirects-filter-empty)');
      let visibleCount = 0;
